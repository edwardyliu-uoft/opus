...
def test_exponential_moving_average_sql_generation():
    metric = ExponentialMovingAverage(
        periods=9,
        source_table="market_events",
        value_column="Price",
    )

    assert metric.name == "market_events_EMA_9"
    assert metric.target_table == "market_events_EMA_9"

    ddl = metric.flink_ddl()
    assert "CREATE TABLE IF NOT EXISTS market_events_EMA_9" in ddl
    assert "snapshot_time TIMESTAMP(3)" in ddl
    assert "'topic' = 'market_events_EMA_9'" in ddl

    dml = metric.flink_dml()
    assert "INSERT INTO market_events_EMA_9" in dml
